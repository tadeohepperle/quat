...
@fragment
fn fs_cutout(in: VertexOutput) -> @location(0) Vec4 {
...
@fragment
fn fs_transparent(in: VertexOutput) -> @location(0) Vec4 {
    let image_color = textureSample(t_diffuse, s_diffuse, in.uv);
    return image_color * in.color;
}

@fragment
fn fs_shine(in: VertexOutput) -> @location(0) Vec4 {
    let image_color = textureSample(t_diffuse, s_diffuse, in.uv);
    let default_color = image_color * in.color;
    let lightness :f32 = (default_color.r + default_color.g + default_color.b) /3.0;
    return  Vec4( (1.0 - default_color.rgb) *0.6, default_color.a * 0.3);
}

@fragment
fn fs_simple(in: VertexOutput) -> @location(0) Vec4 {
    let image_color = textureSample(t_diffuse, s_diffuse, in.uv);
    return image_color * in.color;
}
...
fn more_contrast(color: Vec4, contrast: f32) -> Vec4 {
    let midpoint: f32 = 0.5;
    let adjusted_rgb = Vec3(midpoint) + (color.rgb - Vec3(midpoint)) * contrast;

    let smoothed_rgb = adjusted_rgb * adjusted_rgb * (3.0 - 2.0 * adjusted_rgb);
    return Vec4(smoothed_rgb, color.a);
}
