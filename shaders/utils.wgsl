...
fn world_2d_pos_to_clip_pos(world_pos: vec2<f32>) -> vec4<f32> {
...
fn world_2d_pos_to_clip_pos_with_z(world_pos: vec2<f32>, z: f32) -> vec4<f32> {
    let world_pos_3 = vec3<f32>(world_pos, 1.0);
    let ndc = camera.proj * world_pos_3;
...
fn world_2d_pos_to_clip_pos_3d(pos3: vec3<f32>) -> vec4<f32> {
...
fn screen_pos_to_clip_pos(screen_pos: vec2<f32>) -> vec4<f32> {
    let ndc = screen_pos / frame.screen_size * 2.0  -1.0;
    return  vec4(ndc.x, -ndc.y, 0.0, 1.0);
}

// fn ui_layout_pos_to_clip_pos(ui_layout_pos: vec2<f32>) -> vec4<f32> {
