alias Vec2  = vec2<f32>;
alias Vec3  = vec3<f32>;
alias Vec4  = vec4<f32>;
alias IVec2 = vec2<i32>;
alias Mat4  = mat4x4<f32>;
alias Mat3  = mat3x3<f32>;


@group(0) @binding(0) var<uniform> frame: Frame;
struct Frame {
	screen_size:                  Vec2,
	cursor_pos:                   Vec2,
	xxx:                          Vec4, // experimental values
    total_time:                   f32,
	space_ctrl_shift_alt_pressed: u32, // (bool, bool, bool, bool)
}

@group(1) @binding(0) var<uniform> camera: Camera2D;
struct Camera2D {
    proj:             Mat3, // note: padded like this: (f,f,f,_,  f,f,f,_,  f,f,f,_)
    pos:              Vec2,
    height:           f32, 
}

@group(1) @binding(0) var<uniform> camera3d: Camera3D;
struct Camera3D {
    view_proj:   Mat4,
    view:        Mat4,
    proj:        Mat4,
    view_pos:    Vec4,
    _pad:        Vec4, // because of Odin 32 bytes Mat4 alignment
}


fn world_2d_pos_to_clip_pos(world_pos: Vec2) -> Vec4 {
    let world_pos_3 = Vec3(world_pos, 1.0);
    let ndc = camera.proj * world_pos_3;
    return Vec4(ndc.x, ndc.y, 0.0, 1.0);
}

// same as world_2d_pos_to_clip_pos_3d but the z value has no effect on the screen coordinates, just used for depth buffer (height in z space (orthogonal to ground plane))
fn world_2d_pos_to_clip_pos_with_z(world_pos: Vec2, z: f32) -> Vec4 {
    let world_pos_3 = Vec3(world_pos, 1.0);
    let ndc = camera.proj * world_pos_3;
    return Vec4(ndc.x, ndc.y, z_to_depth(z), 1.0);
}
// same right handed coordinate system as in blender, x and y on the plane, z for height of e.g. walls
const WORLD_UI_Z_SQUASH_FACTOR: f32 = 0.5;

fn world_2d_pos_to_clip_pos_3d(pos3: Vec3) -> Vec4 {
    // military projection, every z step up, gets you half a y step:
    let extended_pos2 = Vec3(pos3.x, pos3.y + pos3.z * WORLD_UI_Z_SQUASH_FACTOR, 1.0);
	let ndc = camera.proj * extended_pos2;
	return Vec4(ndc.x, ndc.y, z_to_depth(pos3.z),1.0);
}


const MAX_SCENE_HEIGHT: f32 = 20.0;
const MAX_SCENE_HEIGHT_2: f32 = MAX_SCENE_HEIGHT * 2.0;
fn z_to_depth(z: f32) -> f32 {
    return z / MAX_SCENE_HEIGHT_2 + 0.5;
}

fn screen_pos_to_clip_pos(screen_pos: Vec2) -> Vec4 {
    let ndc = screen_pos / frame.screen_size * 2.0  -1.0;
    return  vec4(ndc.x, -ndc.y, 0.0, 1.0);
}

// fn ui_layout_pos_to_clip_pos(ui_layout_pos: Vec2) -> Vec4 {
//     let ndc = ui_layout_pos / globals.screen_ui_layout_extent * 2.0  -1.0;
//     return  vec4(ndc.x, -ndc.y, 0.0, 1.0);
// }

const RED : Vec4 = Vec4(1.0, 0.0, 0.0, 1.0);
const GREEN : Vec4 = Vec4(0.0, 1.0, 0.0, 1.0);
const BLUE : Vec4 = Vec4(0.0, 0.0, 1.0, 1.0);
const BLACK : Vec4 = Vec4(0.0, 0.0, 0.0, 1.0);

fn osc(speed: f32, low: f32, high: f32) -> f32 {
    let half_diff = (high - low) / 2.0;
    return sin(frame.total_time * speed) * half_diff + low + half_diff;
}

fn dist_gradient(pt: Vec2, center: Vec2) -> Vec4 {
    let frequency = 20.0;
    let d = length(pt - center); // Compute distance to the target point
    let t = 0.5 + 0.5 * cos(d * frequency); // Create a repeating gradient with cosine
    return Vec4(t, t * 0.5, 1.0 - t, 1.0); // Map to a color gradient (blue to cyan)
}

fn unit_uv_from_idx(idx: u32) -> Vec2 {
    return Vec2(
        f32(((idx << 1) & 2) >> 1),
        f32((idx & 2) >> 1)
    );
}

fn map_unit_uv(u_uv: Vec2, uv_aabb: Vec4) -> Vec2 {
    return Vec2(
        (1.0 - u_uv.x) * uv_aabb.x + u_uv.x * uv_aabb.z,
        u_uv.y * uv_aabb.y + (1.0 - u_uv.y) * uv_aabb.w
    );
}

fn rotate(pos: Vec2, rot: f32) -> Vec2{
    let s = sin(rot);
    let c = cos(rot);
    return Vec2(
        c * pos.x - s * pos.y,
        s * pos.x + c * pos.y,
    );
}

const LIGHT_DIR : Vec3 = Vec3(-0.4, 0.8, 0.2);