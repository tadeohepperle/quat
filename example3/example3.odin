...
main :: proc() {
	...
	skinned_mesh := engine.create_skinned_mesh(triangles, vertices, 2)
	...
	for engine.next_frame() {
		for target in key_and_pose_targets {
			if q.is_key_pressed(target.key) {
				t := q.get_delta_secs() * LERP_SPEED
				current_pose_mix = q.lerp(current_pose_mix, target.pose_mix, t)
				break
			}
		}
		engine.add_ui(q.triangle_picker(&current_pose_mix))
		...
		engine.set_skinned_mesh_bones(skinned_mesh, current_pose[:])
		engine.draw_skinned_mesh(skinned_mesh, {0, 0}, {1, 1, 1, 0.3})
		engine.draw_grid(1, q.Color{1, 1, 1, 0.2})
		if !q.is_key_pressed(.SPACE) {
			engine.set_current_mesh_2d_texture(texture)
			verts, tris, start := engine.access_mesh_2d_write_buffers()
			for v in vertices {
				pos := v.pos
				pos =
		...
		for bone in current_bones {
			engine.draw_gizmos_circle(bone.head, 0.2)
			engine.draw_gizmos_circle(bone.root, 0.3)
			dir := linalg.normalize(bone.head - bone.root)
			perp := Vec2{-dir.y, dir.x} * 0.3
			engine.draw_gizmos_triangle(bone.head, bone.root + perp, bone.root - perp)
		}
		engine.camera_controller_update(&cam)

		file_paths := engine.get_dropped_file_paths()
		if file_paths != nil {
