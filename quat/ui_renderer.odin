...
ui_render :: proc(
	...
	last_proj_idx := max(UiProjectionIdx)
	...
	for &batch, i in batches.batches {
		...
		if set_push_const {
			last_proj_idx = batch.proj_idx
			proj := batches.projections[batch.proj_idx]
			push_const := ui_projection_to_world_ui_push_const(proj, screen_size)
			wgpu.RenderPassEncoderSetPushConstants(
				render_pass,
				{.Vertex, .Fragment},
				0,
				size_of(WorldUiPushConst),
				&push_const,
			)
		}
		...
		if clipping_rect, ok := batch.clipping_rect.(Aabb); ok {
			proj := batches.projections[batch.proj_idx]
			screen_proj, is_screen_proj := proj.(UiScreenProjection)
			assert(is_screen_proj, "only sopports clipping rects on screen projection")
...
legacy_ui_screen_ui_render :: proc(
	batches: UiBatches,
	buffers: UiRenderBuffers,
	rect_pipeline: wgpu.RenderPipeline,
	glyph_pipeline: wgpu.RenderPipeline,
...
ui_rect_pipeline_config :: proc(legacy_screen_render: bool) -> RenderPipelineConfig {
	return RenderPipelineConfig {
		...
		push_constant_ranges = push_const_range(f32, {.Vertex, .Fragment}) if legacy_screen_render else push_const_range(WorldUiPushConst, {.Vertex, .Fragment}),
...
ui_glyph_pipeline_config :: proc(legacy_screen_render: bool) -> RenderPipelineConfig {
	// todo! support 3d!
...
ui_projection_to_world_ui_push_const :: proc(proj: UiProjection, screen_size: Vec2) -> WorldUiPushConst {
	...
	//
	// The clip coordinates are between [-1..1, -1..1, ]
	switch proj in proj {
